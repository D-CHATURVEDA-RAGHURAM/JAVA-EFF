...
const MaterialPriceGrid = () => {
  ...
  const url = "effigo/api/grn/v1/kbl/getMaterialPriceDetails";
  const [materialValue, setMaterialValue] = useState<any>(null);
  const [materialDropDown, setMaterialDropDown] = useState<any>([]);
  const [materialFilter, setMaterialFilter] = useState<any>("");
  const [materialFilterDropDown, setMaterialFilterDropDown] = useState<any>([]);
  const [selectedItemCodes, setSelectedItemCodes] = useState<any>([]);
  ...
  const fetchColumns = async () => {
    try {
      const response: any = await axios.get(
        `${EFFIGO_MATERIAL_PRICE}effigo/api/grn/v1/kbl/getMaterialPriceTitles`,
        {
          headers: {
  ...
  const fetchRows = async (
    ...
    try {
      const response: any = await axios.post(
        ...
        {
          itemCodes: iCodes || selectedItemCodes,
        },
        {
          params: {
            fromDate: fd || formatDateYMD(startDate),
            toDate: ed || formatDateYMD(endDate),
          },
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
  ...
  const fetchDropDown = async () => {
    try {
      const response: any = await axios.get(
        `${EFFIGO_MATERIAL_PRICE}effigo/api/grn/v1/kbl/getMaterialDropdown`,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
      ...
      setMaterialDropDown(response?.data?.data);
      setMaterialFilterDropDown(response?.data?.data);
    } catch (error) {
      console.error("Error fetching data:", error);
    }
  };
  ...
  const onDropFilterChangeHandler = (e: any) => {
    ...
    setMaterialFilter(filterValue);
    const filterDropDownValues = materialDropDown.filter((dropDownData: any) =>
      dropDownData?.materialName
        ?.toLowerCase()
        .includes(filterValue.toLowerCase())
    );
    setMaterialFilterDropDown(filterDropDownValues);
  };
  ...
  const onClearHandler = () => {
    ...
    setMaterialFilterDropDown(materialDropDown);
  ...
  const handleDropDownChange = (event: any) => {
    ...
    setSelectedItemCodes(() => {
      const selectedValues = values.map((value: any) => value?.itemCode);
              ...
              <div className="gridToolbar">
                ...
                <div className="filter-container" onKeyDown={handleKeyPress}>
                  ...
                  <div className="filter dropdown-filter">
                    <label className="dropdown-label">Material Name</label>
                    <MultiSelect
                      data={materialFilterDropDown}
                      filterable={true}
                      placeholder="Select"
              ...
              <GridColumn
                ...
                width={
                  ...
                  column.title.toLowerCase().includes("currency")
                    ? 130
                    : column.title.toLowerCase().includes("description")
                    ? 300
                    : columns.length > 6
                    ? 200
                    : ""
                }
                cell={column?.field === "itemCode" ? linkColumn : undefined}
              />
            ))}
          </Grid>
        )}
  ...
  return (
    <>
      <ExcelExport
        ...
        data={rows}
        filterable={true}
        fileName="Material Price Indices"
      >
        {columns.map((column: any) => {
          return (
            <ExcelExportColumn
...
export default MaterialPriceGrid;

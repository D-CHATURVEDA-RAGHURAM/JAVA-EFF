...
public final class POConstant {
	...
	public enum POErrorCodes {
		...
		private POErrorCodes(String code, String description) {
			this.errCode = code;
		...
		public static POErrorCodes fromErrorCode(String value) {
			return Stream.of(POErrorCodes.values()).filter(prerr -> prerr.getErrCode().equals(value)).findFirst().orElse(null);
		}
	...
	public enum POSuccessCodes {
		PO_CREATED_SUCCESSFULLY("PO created successfully"), PO_UPDATE_SUCCESSFULLY(
				"PO updated successfully"), PO_AMEND_SUCCESSFULLY(
						"PO amended successfully"), PO_CANCEL_SUCCESSFULLY("PO cancelled successfully");

		private final String value;

		POSuccessCodes(final String value) {
			this.value = value;
		}
	...
	public enum POActiveCodes {
		ITEM_STATUS_ACTIVE ("1"),

		ITEM_STATUS_IN_ACTIVE ("0"),

		ITEM_STATUS_CLOSED ("2"),

		ITEM_STATUS_DELETE("X");
		private final String value;

		POActiveCodes(final String value) {
			this.value = value;
		}
